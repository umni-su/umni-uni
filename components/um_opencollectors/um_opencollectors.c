...
{
    return (state == UM_OC_STATE_ON) ? 1 : 0;
}
...
esp_err_t um_opencollectors_init(void)
{
    ...
    if (channels[UM_OC_CHANNEL_1].gpio_num >= 0)
    {
        ...
        else
        {
            ...
            if (saved_states >= 0)
            {
                channels[UM_OC_CHANNEL_1].state = (saved_states & OC1_STATE_MASK) ? UM_OC_STATE_ON : UM_OC_STATE_OFF;
            }
            else
            {
    ...
    if (channels[UM_OC_CHANNEL_2].gpio_num >= 0)
    {
        ...
        else
        {
            ...
            if (saved_states >= 0)
            {
                channels[UM_OC_CHANNEL_2].state = (saved_states & OC2_STATE_MASK) ? UM_OC_STATE_ON : UM_OC_STATE_OFF;
            }
            else
            {
