...
{
    ESP_LOGI(REST_TAG, "Запрос статического файла: %s", req->uri);
...
{
    ESP_LOGI(WEBSERVER_TAG, "Запуск UM веб-сервера");
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(WEBSERVER_TAG, "Ошибка запуска сервера: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    ESP_LOGI(WEBSERVER_TAG, "Веб-сервер запущен успешно");
    return ESP_OK;
}
...
{
    if (server) {
        ESP_LOGI(WEBSERVER_TAG, "Остановка веб-сервера");
        httpd_stop(server);
        server = NULL;
    }
    return ESP_OK;
